...
export const validateAlias = (alias: string): { valid: boolean; error?: string } => {
  const sanitized = sanitizeAlias(alias);
  
  if (!sanitized) return { valid: false, error: "Alias não pode ser vazio" };
  if (sanitized.length < 2) return { valid: false, error: "Alias precisa ter pelo menos 2 caracteres" };
  if (/^\d+$/.test(sanitized)) return { valid: false, error: "Alias não pode ser apenas números" };
  
  const forbiddenPatterns = [
    /^(admin|root|api|www|mail)$/i,
    /^[_-]+$/,
  ];
...
export const reservedPaths = [
  "", "shorten", "stats", "docs", "ping", "login", "register", "auth",
  "signin", "signup", "logout", "api", "_next", "_vercel", "vercel",
  ...
  "404", "500", "error", "not-found", "webhook", "webhooks", "callback",
  "oauth", "health", "status", "metrics", "monitoring", "ping", "www",
  "mail", "email", "ftp", "blog", "news", "shop", "store", "administrator",
  "manage", "management", "console", "download", "upload", "file", "files", "media"
];
